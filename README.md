...
| `build.bat` | Запуск сборки в один клик (Setup + Portable + Zip) |
| `scripts/build-native.ps1` | Скрипт сборки нативного приложения и установщиков NSIS |
| `scripts/installer.nsi` | Сценарий NSIS Setup-установщика (выбор папки, ярлыки, служба) |
| `scripts/portable.nsi` | Сценарий NSIS Portable-лаунчера (быстрый кэшированный запуск) |
...
| `scripts/make-icon.ps1` | Генерация `assets/icon.png` и multi-res `assets/icon.ico` |
...
Собрать все варианты в один клик:
```bat
build.bat
```
...
Результаты сборки сохраняются в папке `dist/`:
- **`dist\MrOpenVPNClient-1.3.0-setup.exe`** — полноценный NSIS-установщик:
  - Выбор директории установки (по умолчанию `C:\Program Files\MrOpenVPNClientWindows`);
  - Опция «Создать ярлык на рабочий стол»;
  - Опция «Создать ярлык в меню пуск»;
  - Опция «Сразу установить службу (обязательно, но можно потом)»;
  - Запись в «Установку и удаление программ» Windows и чистый деинсталлятор.
- **`dist\MrOpenVPNClient-1.3.0-portable.exe`** — сверхбыстрый одиночный исполняемый файл без установки (кэширует запуск в LocalAppData).
- **`dist\MrOpenVPNClient-1.3.0-portable.zip`** — классический ZIP-архив с портативной папкой приложения.
- **`dist-native\`** — исходный распакованный рабочий каталог приложения.
...
- **Маршрутизация**: поддержка Full Tunnel (`redirect-gateway def1` и `block-outside-dns`)
  и Split Tunnel.
- **Поведение в трее и Single-Instance**:
  - При закрытии окно сворачивается в системный трей.
  - Повторный запуск или клик по ярлыку (когда приложение уже свёрнуто в трей) мгновенно разворачивает существующее окно на передний план.
  - Автоподключение к последнему профилю, переподключение при смене сети,
